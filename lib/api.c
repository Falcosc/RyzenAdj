...
EXP uint32_t get_table_addr(ryzen_access ry)
{
	...
	if(!data){
		//Raven and Picasso don't get table refresh on the very first transfer call after boot
...
EXP int CALL refresh_pm_table(ryzen_access ry)
{
	...
	int resp = smu_service_req(ry->psmu, transfer_table_msg, &args);
	if (resp == REP_MSG_OK) {
		return 1;
	} else if (resp == REP_MSG_UnknownCmd) {
		...
		printf("Transfer table was rejected\n");
...
EXP float CALL get_max_gfxclk_freq(ryzen_access ry){return NAN;}
EXP float CALL get_min_gfxclk_freq(ryzen_access ry){return NAN;}
EXP float CALL get_max_socclk_freq(ryzen_access ry){return NAN;}
EXP float CALL get_min_socclk_freq(ryzen_access ry){return NAN;}
EXP float CALL get_max_fclk_freq(ryzen_access ry){return NAN;}
EXP float CALL get_min_fclk_freq(ryzen_access ry){return NAN;}
EXP float CALL get_max_vcn(ryzen_access ry){return NAN;}
EXP float CALL get_min_vcn(ryzen_access ry){return NAN;}
EXP float CALL get_max_lclk(ryzen_access ry){return NAN;}
EXP float CALL get_min_lclk(ryzen_access ry){return NAN;}
EXP float CALL get_prochot_deassertion_ramp(ryzen_access ry){return NAN;}
